define [
	...
	"display/layout/ZoeLayout"
	"display/render/BackgroundRender"
	"display/render/ImageRender"
], (DisplayObject,Vendor,BasicLayout,ZoeLayout,BackgroundRender,ImageRender) ->
	...
	class MovieClip extends DisplayObject
		...
		constructor: (data, options=null) ->
			...
			if options?.renderMethod?
				switch options.renderMethod
					when MovieClip.RENDER_METHODS.background
						@_renderEngine = new BackgroundRender(@_layoutEngine.images)
					when MovieClip.RENDER_METHODS.image
		...
		_initOptions: (options) ->
			super(options)
			@_setOptions(options,MovieClip.INIT_DEFAULTS)
